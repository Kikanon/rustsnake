...
impl MyRustSnakeGame
 {
    ...
    fn game_logic(&mut self){
        ...
        while self.elapsed_time >= update_interval {
            self.game_update();
    ...
    fn draw_player(
        ...
        let head_img = match self.game_state.player_direction {
        ...
        self.draw_at_cell(ui, head_img.clone(), self.game_state.player_position, self.map_size);
    }

    fn game_update(&mut self){

        self.game_state.body_parts.push_front(self.game_state.player_position);
        if self.game_state.body_parts.len() > self.game_state.snake_length as usize {
            self.game_state.body_parts.pop_back();
        }

        match self.game_state.player_direction {
            Direction::TopDown => {
                self.game_state.player_position.0 = (self.game_state.player_position.0 + 1) % self.map_size.0 as u16;
            },
            Direction::BottomUp => {
                self.game_state.player_position.0 = (self.game_state.player_position.0 + self.map_size.0 as u16 - 1) % self.map_size.0 as u16;
            },
            Direction::LeftToRight => {
                self.game_state.player_position.1 = (self.game_state.player_position.1 + 1) % self.map_size.1 as u16;
            },
            Direction::RightToLeft => {
                self.game_state.player_position.1 = (self.game_state.player_position.1 + self.map_size.1 as u16 - 1) % self.map_size.1 as u16;
            },
        }
        // Here you would also check for collisions with the walls, food, etc.
